...
- **`arm64.js`** — the opcode table, decode masks, and now `memo` — 4MB of `Uint8Array` RAM with a `DataView` on top for word/half-word/byte access.
...
- `ldr`, `str`, `ldrh` — load/store into real RAM through a base register: `ldr x1 [x2]`.
...
- Byte-level load/store, full assembly script support (multi-line, not just one command at a time).
